...
plot_chirodefo <- mort_inverts %>%
  ...
  stat_cor(cor.coef.name = "r",label.sep = ", ", r.accuracy = 0.01, p.accuracy = 0.01, label.x =2.8, label.y = 43) +
...
chiro_mort1 <- chiro_mort + geom_hline(yintercept=7.4, linetype="dashed", color = "red", size=2) + 
  annotate('rect', xmin=0, xmax=3, ymin=-6, ymax= 20.8, alpha=.4, fill='grey', color='white')  # specify size based on 2 SD (Celine's reccomendation based on a paper )
...
plot(mod, res.type="pit.norm", which=3, caption=c("Residuals vs Fitted", "Normal Q-Q", 
  "Scale-Location", "Cook's distance"), overlay=TRUE, 
  n.vars=Inf, var.subset=NULL, sub.caption=NULL)

set.seed(123)  # reproducibility

anova_res <- anova(mod_test, p.uni = "adjusted", nBoot = 999)
print(anova_res)
...
effect_sizes <- data.frame(
  taxon = colnames(coefs),
  coef = coef(mod)["groupUndefoliated",]
)

anova_uni <- anova_res$uni.p
effect_sizes$p <- anova_uni["group",]

effect_sizes$signif <- cut(effect_sizes$p,
   breaks = c(-Inf, 0.05, Inf),
   labels = c("p < 0.05", "ns")
)

effect_sizes_sig <- effect_sizes %>%
  filter(signif != "ns") %>%
  arrange(desc(coef))

effect_sizes_sig$taxon <- factor(effect_sizes_sig$taxon, levels = rev(effect_sizes_sig$taxon))

effect_sizes_sig <- effect_sizes_sig %>%
  mutate(Direction = ifelse(coef < 0, "Defo", "NonDefo"))

effect_sizes_sig <- effect_sizes_sig %>%
  filter(taxon != "unknown")

lmmany_plot <- ggplot(effect_sizes_sig, aes(x = coef, y = taxon, fill = Direction)) +
  geom_col(width = 0.7) +
  geom_vline(xintercept = 0, linetype = "dashed", color = "gray40") +
  scale_x_continuous(
    name = "Effect size (difference in Hellinger-transformed abundance)") +
   # limits = c(min_log_or, max_log_or),
  #  breaks = seq(min_log_or, max_log_or, 1)
#  ) +
  scale_y_discrete(name = "Significant Taxa (p <0.05)") +
  scale_fill_manual(values = c("Defo" = "#440154", "NonDefo" = "#b5de2b"), 
                        labels = c("Defo" = "Defoliated", "NonDefo" = "Non-defoliated")) +
  labs(
...
gammaplot <- ggplot(gamma_df, aes(x=Category, y=count, fill = Category)) + geom_bar(stat = "identity") + theme_bw() + guides(fill=guide_legend(title="Number of Families"))  + scale_fill_manual(name = "Number of Families", values = c("Unique to Defoliated" = "#440154", "Unique to Undefoliated" = "#b5de2b", Shared = "grey")) + 
  scale_x_discrete(
    labels = c(
      ...
      "Unique to Undefoliated" = "Undefoliated Only",
      "Shared" = "Present in Both Groups")) + ylab("Number of Families") + xlab("Presence-Absence per Group") +  theme_minimal(base_size = 16) + theme(legend.position = "none")
