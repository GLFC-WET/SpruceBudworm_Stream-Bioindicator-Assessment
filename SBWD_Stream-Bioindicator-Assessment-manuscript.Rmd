...
- it is organized into two main sections:
  ...
  - Part 2: Invert Community Data Analysis (2020-2023)
    ...
    - Section B: Comparison to undefoliated CABIN Watersheds
...
waterchem_kicks <- waterchem_wide %>%
  ...
  dplyr::filter(date.collected %in% c("2021-09-17", "2021-09-18", "2021-09-19", "2022-09-08", "2022-09-09", "2023-09-11", "2023-09-12", "2023-09-13"))
```
...
inverts_Test <- inverts_family %>%
  mutate(sample = paste(year, site, sep = "_")) %>%
    ungroup() %>%
  dplyr::select(Family, sample, fcount) %>%
 # dplyr::select(-year) %>%
...
plot_pca_all <- ggplot() +
 ...
 # geom_text_repel(data = sites, aes(x = PC1, y = PC2, label = sample), color = "black") + 
  ...
  guides(shape = guide_legend(title = "Mortality"))  + theme_minimal() + theme(legend.position = "bottom", legend.box = "vertical", panel.grid.major = element_blank(), panel.grid.minor = element_blank())
...
loading_flow<- sig.env_all %>%
  filter(env.variables == "flow.m3s")

loading_temp <- sig.env_all %>%
  filter(env.variables == "mean_sept_temp")

loading_cond <- sig.env_all %>%
  filter(env.variables == "cond.umho.cm")

loading_defo <- sig.env_all %>%
  filter(env.variables == "cdefo5y")

loading_elev <- sig.env_all %>%
  filter(env.variables == "Elevation_Mean_metres")

loading_doc <- sig.env_all %>%
  filter(env.variables == "DOC.mgL")

loading_spfir <- sig.env_all %>%
  filter(env.variables == "spfir")

loading_pH <- sig.env_all %>%
  filter(env.variables == "pH")

loading_tn <- sig.env_all %>%
  filter(env.variables == "TN.mgL")

final_pca_all_plot <- plot_pca_all + geom_segment(data = sig.env_all, aes(x = 0, y = 0, xend = PC1, yend = PC2), arrow = arrow(length = unit(0.25, "cm")), colour = "black", lwd = 0.3) + 
  #geom_text_repel(data = loading_rest, aes(x = PC1, y = PC2), colour = "black", fontface = "bold", label = loading_rest$pretty_label) + 
geom_text(data = loading_flow, aes(PC1, PC2), colour = "black", fontface = "bold", label = loading_flow$pretty_label, size =4, nudge_x = 0.13, nudge_y = 0.03) + 
  geom_text(data = loading_temp, aes(PC1, PC2), colour = "black", fontface = "bold", label = loading_temp$pretty_label, size =4, nudge_x = -0.15, nudge_y = 0.05) + 
  geom_text(data = loading_cond, aes(PC1, PC2), colour = "black", fontface = "bold", label = loading_cond$pretty_label, size =4, nudge_y = -0.05) + 
  geom_text(data = loading_defo, aes(PC1, PC2), colour = "black", fontface = "bold", label = loading_defo$pretty_label, size =4, nudge_y = -0.05) + 
  geom_text(data = loading_elev, aes(PC1, PC2), colour = "black", fontface = "bold", label = loading_elev$pretty_label, size =4, nudge_y = 0.05, nudge_x = 0.05) + 
  geom_text(data = loading_doc, aes(PC1, PC2), colour = "black", fontface = "bold", label = loading_doc$pretty_label, size =4, nudge_y = 0.03, nudge_x = -0.01) + 
    geom_text(data = loading_spfir, aes(PC1, PC2), colour = "black", fontface = "bold", label = loading_spfir$pretty_label, size =4, nudge_y = -.04, nudge_x = 0) +
    geom_text(data = loading_pH, aes(PC1, PC2), colour = "black", fontface = "bold", label = loading_pH$pretty_label, size =4, nudge_y = 0, nudge_x = -.05) +
    geom_text(data = loading_tn, aes(PC1, PC2), colour = "black", fontface = "bold", label = loading_tn$pretty_label, size =4, nudge_y = 0.04, nudge_x = 0) 
...
inverts23_nomort <- mort_inverts23 %>%
  dplyr::filter(mort_cat == "NoMort")

inverts_nomort <- mort_inverts %>%
    dplyr::filter(mort_cat == "NoMort")
```
...
plot_EPTdefo <- mort_inverts %>%
        ggscatter(x = "cdefo5y", y = "perc_EPT", color = "year", size = 4, shape = "mort_cat",
                  ...
                  cor.coef = FALSE, cor.method = "pearson") + #cor.coef.coord = c(3, 100), cor.coef.size = 5) + 
  stat_cor(cor.coef.name = "r",label.sep = ", ", r.accuracy = 0.01, p.accuracy = 0.01, label.x = 3, label.y = 100) +
                  facet_wrap(~year) + scale_color_brewer(palette = "Dark2", guide = "none") + geom_smooth(data = mort_inverts23, method = "lm", color = "#7570b3", fill = "#7570b3") + geom_point(data = mort_inverts23, color = "#7570b3", fill = "#7570b3", size = 4, aes(shape = "mort_cat"))+ labs(x = "Cumulative Defoliation", y = "% EPT") + theme_bw(base_size = 20) + theme(panel.border = element_blank(), panel.grid.major = element_blank(),
panel.grid.minor = element_blank(), axis.line = element_line(colour = "black")) + scale_shape_manual(values = c("NoMort" = 16, "Mort" = 17, "LowMort" = 15),  labels = c("NoMort" = "< 10% Mortality", "Mort" = "> 30% Mortality", "LowMort" = "10 - 29.9% Mortality")) + 
  guides(color = guide_legend(title = "Year", nrow = 3, byrow = TRUE),
         shape = guide_legend(title = "Tree Mortality", nrow = 3, byrow = TRUE)) + theme(legend.position = "bottom", base_size = 16)

#chiro
plot_chirodefo <- mort_inverts %>%
        ggscatter(x = "cdefo5y", y = "perc_chironomid", color = "year", size = 4, shape = "mort_cat",
                  #add = "reg.line", conf.int = TRUE, 
                  cor.coef = FALSE, cor.method = "pearson") + #, cor.coef.coord = c(3, 40), cor.coef.size = 5) +
  stat_cor(cor.coef.name = "r",label.sep = ", ", r.accuracy = 0.01, p.accuracy = 0.01, label.x =2.8, label.y = 40) +
                  facet_wrap(~year) + scale_color_brewer(palette = "Dark2") + geom_smooth(data = mort_inverts23, method = "lm", color = "#7570b3", fill = "#7570b3") + geom_point(data = mort_inverts23, color = "#7570b3", fill = "#7570b3", size = 4, aes(shape = "mort_cat")) + labs(x = "Cumulative Defoliation", y = "% Chironomidae") + theme_bw(base_size = 20) + theme(panel.border = element_blank(), panel.grid.major = element_blank(),
...
#EPT
# EPT defo 
plot_EPTdefo1 <- inverts_nomort %>%
          ggplot(aes(x = cdefo5y, y = perc_EPT, color = year)) +
  geom_point(size = 1) +
stat_poly_eq(use_label(c("R2", "p")), color = "black", size = 5) + 
   stat_poly_line(data = inverts23_nomort) +
  facet_wrap(~year) + scale_color_brewer(palette = "Dark2") + scale_fill_brewer(palette = "Dark2") + labs(x = "Cumulative Defoliation", y = "%EPT") + theme_bw(base_size = 20) + theme(panel.border = element_blank(), panel.grid.major = element_blank(),
panel.grid.minor = element_blank(), axis.line = element_line(colour = "black"), legend.position = "bottom") +
  geom_point(aes(shape = mort_cat), size = 4) +
scale_shape_manual(values = c("NoMort" = 16, "Mort" = 17, "LowMort" = 15),  labels = c("NoMort" = "< 10% Mortality", "LowMort" = "10 - 29.9% Mortality", "Mort" = "> 30% Mortality")) + 
  guides(color = guide_legend(title = "Year", nrow = 3, byrow = TRUE),
...
plot_chirodefo1 <- inverts_nomort %>%
            ggplot(aes(x = cdefo5y, y = perc_chironomid, color = year)) +
  geom_point(size = 1) +
stat_poly_eq(use_label(c("R2", "p")), color = "black", size = 5) + 
   stat_poly_line(data = inverts23_nomort) +
  facet_wrap(~year) + scale_color_brewer(palette = "Dark2") + scale_fill_brewer(palette = "Dark2") + labs(x = "Cumulative Defoliation", y = "%Chironomidae") + theme_bw(base_size = 20) + theme(panel.border = element_blank(), panel.grid.major = element_blank(),
panel.grid.minor = element_blank(), axis.line = element_line(colour = "black"), legend.position = "bottom") +
  geom_point(aes(shape = mort_cat), size = 4) +
scale_shape_manual(values = c("NoMort" = 16, "Mort" = 17, "LowMort" = 15),  labels = c("NoMort" = "< 10% Mortality", "LowMort" = "10 - 29.9% Mortality", "Mort" = "> 30% Mortality")) + 
  guides(color = guide_legend(title = "Year", nrow = 3, byrow = TRUE),
...
EPT_mort <- ggplot(mort_inverts23, aes(x=mort_cat, y=perc_EPT, fill=mort_cat)) +
   ...
   # stat_summary(fun.y=mean, geom="point", shape=20, size=14, color="red", fill="red") +
    theme(legend.position="none") + labs(x = "Study Watershed Type", y = "% EPT") +
...
chiro_mort <- ggplot(mort_inverts23, aes(x=mort_cat, y=perc_chironomid, fill=mort_cat)) +
    geom_boxplot() +
   # stat_summary(fun.y=mean, geom="point", shape=20, size=14, color="red", fill="red") +
    theme(legend.position="none") + labs(x = "Study Watershed Type", y = "% Chironomidae") + geom_jitter(width = 0.2) +
...
```{r, eval = FALSE}
### EPT 
# check assumptions of lm21 
div21 <- mort_inverts %>%
 filter(year == "2021")

mod21 <- lm(data = div21, perc_EPT ~ cdefo5y)

par(mfrow = c(2, 2))
plot(mod21)

# check assumptions of lm22 
div22 <- mort_inverts %>%
 filter(year == "2022")

mod22 <- lm(data = div22, perc_EPT ~ cdefo5y)

par(mfrow = c(2, 2))
plot(mod22)

# check assumptions of lm23 
div23 <- mort_inverts %>%
 filter(year == "2023")

mod23 <- lm(data = div23, perc_EPT ~ cdefo5y)
...
mod21 <- lm(data = div21, perc_chironomid ~ cdefo5y)
...
mod22 <- lm(data = div22, perc_chironomid ~ cdefo5y)
...
mod23 <- lm(data = div23, perc_chironomid ~ cdefo5y)

par(mfrow = c(2, 2))
plot(mod23)

mod23 <- lm(data = inverts23_nomort, perc_chironomid ~ cdefo5y)

par(mfrow = c(2, 2))
plot(mod23)

mod23 <- lm(data = inverts23_nomort, perc_EPT ~ cdefo5y)
...
## EPT % 
mean(inverts_EPT2$perc_EPT) #mean is 84.7 
...
# just calculate the mean and 95% CIs across ref sites and add it in 
## percent chiro
mean(inverts_EPT2$perc_chironomid) #7.4 

ci <- t.test(inverts_EPT2$perc_chironomid)$conf.int
ci
# 5.4 to 9.4

2*sd(inverts_EPT2$perc_chironomid) #13.4
#range -6 to 20.8
7.4+13.4
7.4-13.4
...
EPT_defo_final <- ggarrange(plot_EPTdefo1, plot_chirodefo1,
        labels = c("A", "B"),
...
EPT_mort_final <- ggarrange(EPT_mort1, chiro_mort1,
        labels = c("A", "B"),
         font.label=list(size = 16, color = "black", face = "bold"),
     common.legend = TRUE,
         legend = "none",
...
plot_pca_all1 <- plot_pca_all +
  ...
  theme_minimal()  + theme(legend.position = "bottom",  legend.box = "vertical", legend.box.just = "left", panel.grid.major = element_blank(), panel.grid.minor = element_blank())
...
### manylm instead of PERMANOVA 

```{r}

# try mvabund 
# this is not as affected by dispersion, also gives us some individual taxa responses
#used manyGLM on raw counts. works best and meets all assumptions 

library(mvabund)

# samples in rows, species in cols 

comm_mv <- mvabund(round(all_inverts))
...
sampleID <- as.data.frame(row.names(all_inverts_hel))

sampleID <- sampleID %>%
  rename(sample = 'row.names(all_inverts_hel)')

sampleID <- sampleID %>%
  left_join(all_metadata, by = "sample")

group <- factor(sampleID$Site_Type, levels = c("Defoliated", "Undefoliated"))

mod <- manyglm(comm_mv ~ group)

#check assumptions, all look good. just switch out which to 1:4
plot(mod, res.type="pit.norm", which=1, caption=c("Residuals vs Fitted", "Normal Q-Q", 
  "Scale-Location", "Cook's distance"), overlay=TRUE, 
  n.vars=Inf, var.subset=NULL, sub.caption=NULL)

set.seed(123)  # reproducibility

anova_res <- anova(mod, p.uni = "adjusted", nBoot = 999)
print(anova_res)
...
gamma_df <- data.frame(
  ...
  count = c(4, 45, 23),
  Category = rep(c("Unique to Defoliated", "Unique to Undefoliated", "Shared"), times = 1)
)

#gamma_df <- data.frame(
#  year = rep(c(2021, 2022, 2023), each = 3),
#  count = c(2, 46, 24, 2, 45, 25, 0, 46, 24),
#  Category = rep(c("Unique to Defoliated", "Unique to Undefoliated", "Shared"), times = 3))


#gamma_df$year <- as.factor(gamma_df$year)

gamma_df$Category <- factor(gamma_df$Category, levels = c("Unique to Defoliated", "Unique to Undefoliated",  "Shared" ))

# ok now need to plot this!
#position = "stack", stat = "identity"

gammaplot <- ggplot(gamma_df, aes(x=Category, y=count, fill = Category)) + geom_bar(stat = "identity") + theme_bw() + guides(fill=guide_legend(title="Number of Families"))  + scale_fill_manual(name = "Number of Families", values = c("Unique to Defoliated" = "#440154", "Unique to Undefoliated" = "#b5de2b", Shared = "grey")) + 
  scale_x_discrete(
    labels = c(
      "Unique to Defoliated" = "Defoliated\nOnly",
      "Unique to Undefoliated" = "Undefoliated\nOnly",
      "Shared" = "Present in\nBoth Groups")) + ylab("Number of Families") + xlab("Presence-Absence per Group") +  theme_minimal(base_size = 16) + theme(legend.position = "none", panel.grid.major = element_blank(), panel.grid.minor = element_blank())
...
- not really differential abundance becauese we are only interested in presence absence, but it is similar
...
daa_plot <- ggplot(sig, aes(x = log_or, y = reorder(Taxon, log_or), fill = Direction)) +
  ...
  scale_fill_manual(values = c("Defo" = "#440154", "Non" = "#b5de2b"), 
                        labels = c("Defo" = "Defoliated", "Non" = "Undefoliated")) +
  labs(
  #  title = "Significant Taxa by Fisher's Exact Test",
  # subtitle = paste("FDR-adjusted p-value <", alpha),
    fill = "Site Type"
  ) +
  theme_minimal(base_size = 16) +  theme(legend.position = "bottom", panel.grid.major = element_blank(), panel.grid.minor = element_blank())
...
#### Supplemental Environmental PCA (site data only, no community data)

## Reference watershed habitat data 

```{r}
RefHabitat <- read.csv("//132.156.200.8/WatershedEcol_b/wet_personal_workspace/MaddieMcCaig/SBWD/CABIN_Analyses/ReferenceData/RefData_Filtered.csv", col.names = c("site", "date.collected", "year", "depth_mean.cm", "depth.bankful", "depth_max.cm", "flow_mean.ms", "flow_max.ms", "bankfull_width.m", "wetted_width.m", "alk.mgL", "DO.mgL", "pH", "water_temp.C", "TP.mgL", "notes", "suspect"))

RefHabitat <- RefHabitat %>%
  select(-depth.bankful, -alk.mgL, -TP.mgL) # remove depth.bankful and alkalinity - too many NAs

# need consistent units with our SBWD
# change cm to m
# change flow m/s to discharge (m3/s) by multiplying by depth and width 

RefHabitat <- RefHabitat %>%
  mutate(depth_mean.m = depth_mean.cm/100) %>%
  mutate(depth_max.m = depth_max.cm/100) %>%
  mutate(flow_mean.m3s = flow_mean.ms*depth_mean.m*wetted_width.m)

RefHabitat <- RefHabitat %>%
  select(-depth_mean.cm, -depth_max.cm, -flow_mean.ms)

#remove 9999 from DO 
RefHabitat[27, "DO.mgL"] <- NA

### PGA001 2016 and BDR 2016 have no habitat data, will be excluded
RefHabitat <- RefHabitat[-c(34, 50), ]
#### DO has 7 NA entries, all other variables less than 5. leave all in for now 


### Also join watershed characteristics 
Ref_watershed_charac <- read.csv("//132.156.200.8/WatershedEcol_b/wet_personal_workspace/MaddieMcCaig/SBWD/CABIN_Analyses/ReferenceData/Reference_watershed_characteristics.csv")

RefHabitat <- RefHabitat %>%
  left_join(Ref_watershed_charac, by = "site")

### data has been updated with corrected depths from project lead

```

```{r}

## also pull in wetted width data for SBWD
###NOTE the temp at time of kick from the stage data was added to this spreadsheet for 2022 because we had no YSI data
## ysi values match up well to stage in other years so fine to use 
SBWD_width_depth <- read.csv("//132.156.200.8/WatershedEcol_b/wet_personal_workspace/MaddieMcCaig/SBWD/CABIN_Analyses/SBWD_wettedwidth_depth.csv")

SBWD_width_depth <- SBWD_width_depth %>%
  rename(date.collected = date)

SBWD_width_depth$date.collected <- as.Date(SBWD_width_depth$date.collected)
## join to stream hab kicks
stream_habitat_kicks$date.collected <- as.Date(stream_habitat_kicks$date.collected )

stream_habitat_kicks <- stream_habitat_kicks %>%
  left_join(SBWD_width_depth, by = c("site", "date.collected"))

SBWD_habitat_comparison <- stream_habitat_kicks %>%
  dplyr::select(site, year, date.collected, flow.m3s, wetted_width.m, depth_mean.m, pH, ysi.ph, ysi.do.mgL, ysi.water.temp.c, temp.C)

#join the two temp cols together
SBWD_habitat_comparison <- SBWD_habitat_comparison %>%
  mutate(water_temp.C = coalesce(ysi.water.temp.c, temp.C)) %>%
    mutate(pH = coalesce(ysi.ph, pH)) %>%
  dplyr::select(-ysi.ph, -ysi.water.temp.c, -temp.C)

#join watershed charac too 
SBWD_habitat_comparison <- SBWD_habitat_comparison %>%
  left_join(watershed_characteristics %>% dplyr::select(1,5,8,21), by = "site") 

```

### ok join things together 

```{r}

RefHabitat <- RefHabitat %>%
  dplyr::select(site, year, date.collected, flow_mean.m3s, wetted_width.m, depth_mean.m, pH, water_temp.C, Catchment_Area_km2, Elevation_Mean_metres, SpFir_Perc) %>%
    rename(spfir = SpFir_Perc) %>%
  rename(flow.m3s = flow_mean.m3s) %>%
    mutate(SiteType = "NonDefo") %>%
  mutate(date.collected = as.Date(date.collected))

#keep 2022, skip DO
SBWD_habitat_comparison <- SBWD_habitat_comparison %>%
  filter(year %in% c("2021", "2022", "2023")) %>%
  dplyr::select(-ysi.do.mgL) %>%
  mutate(SiteType = "Defo")


Habitat <- rbind(RefHabitat, SBWD_habitat_comparison)

Habitat_full <- rbind(RefHabitat, SBWD_habitat_comparison)
#write.csv(Habitat_full, "//132.156.200.8/WatershedEcol_b/wet_personal_workspace/MaddieMcCaig/SBWD/CABIN_Analyses/StreamHabitat_full.csv", row.names = FALSE)
...
```{r}
Habitat <- Habitat %>%
  mutate(sample = ifelse(SiteType == "Defo", paste0(year, "_", site), paste0(site, "_", year)))

#select only numerics
all_metadata1 <- Habitat %>%
  dplyr::select(4:11)

row.names(all_metadata1) <- Habitat$sample

all_metadata_clean <- na.omit(all_metadata1)

pca_env <- prcomp(all_metadata_clean, scale = TRUE)
...
scores$Site_Type <- as.factor(scores$Site_Type)
...
xlab <- round(var_explained[1] * 100, 1)
ylab <- round(var_explained[2] * 100, 1)

x_lab <- paste0("PCA1 ", xlab, " %")
y_lab <- paste0("PCA2 ", ylab, " %")

# make pretty labels
pretty_labels <- c(
  "flow.m3s" = "Stream Flow", 
  "wetted_width.m" = "Wetted Width",
  "depth_mean.m" = "Stream Depth", 
  "pH" = "pH", 
  "water_temp.C" = "Stream Temperature", 
  "Catchment_Area_km2" = "Catchment Area", 
  "Elevation_Mean_metres" = "Elevation", 
  "spfir" = "% Spruce-Fir")

loadings$env.variables <- row.names(loadings)
# Apply the mapping
loadings$pretty_label <- pretty_labels[loadings$env.variables]

# need to adjust labels individually 
loading_depth <- loadings %>%
  filter(env.variables == "depth_mean.m")

loading_temp <- loadings %>%
  filter(env.variables == "water_temp.C")

loading_catch <- loadings %>%
  filter(env.variables == "Catchment_Area_km2")

loading_pH <- loadings %>%
  filter(env.variables == "pH")

loading_spfir <- loadings %>%
  filter(env.variables == "spfir")

loading_flow <- loadings %>%
  filter(env.variables == "flow.m3s")

loading_width <- loadings %>%
  filter(env.variables == "wetted_width.m")

loading_elev <- loadings %>%
  filter(env.variables == "Elevation_Mean_metres")

ggplot() +
  geom_point(data = scores, aes(PC1, PC2,color = Site_Type, fill = Site_Type), size = 4) +
  geom_segment(data = loadings,
               aes(x = 0, y = 0, xend = PC1*2, yend = PC2*2),
               arrow = arrow(length = unit(0.25, "cm"))) +
  theme_minimal(base_size=16) + labs(x = x_lab, y = y_lab) + scale_color_manual(values = c("Defoliated" = "#440154", "Undefoliated" = "#b5de2b")) +
  stat_ellipse(data = scores, aes(x = PC1, y = PC2, linetype = Site_Type, color = Site_Type), type = "norm", level = 0.95) + 
  geom_text(data = loading_elev, aes(PC1*2, PC2*2, colour = "black", fontface = "bold", label = pretty_label, nudge_y = -0.1, nudge_x = -.3), size =4) +
  geom_text(data = loading_depth, aes(PC1*2, PC2*2, colour = "black", fontface = "bold", label = pretty_label, nudge_x = 0.4, nudge_y = -0.1), size =4) +
  geom_text(data = loading_width, aes(PC1*2, PC2*2, colour = "black", fontface = "bold", label = pretty_label, nudge_x = -0.6, nudge_y = 0.1), size =4) +
  geom_text(data = loading_temp, aes(PC1*2, PC2*2, colour = "black", fontface = "bold", label = pretty_label, nudge_y = 0.1), size =4) + geom_text(data = loading_catch, aes(PC1*2, PC2*2, colour = "black", fontface = "bold", label = pretty_label, nudge_x = -0.8, nudge_y = 0.1), size =4) + geom_text(data = loading_pH, aes(PC1*2, PC2*2, colour = "black", fontface = "bold", label = pretty_label, nudge_y = -0.1, nudge_x = 0.1), size =4) + 
  geom_text(data = loading_spfir, aes(PC1*2, PC2*2, colour = "black", fontface = "bold", label = pretty_label, nudge_x = 0.7, nudge_y = 0.1), size =4) + geom_text(data = loading_flow, aes(PC1*2, PC2*2, colour = "black", fontface = "bold", label = pretty_label, nudge_x = -0.6), size =4)
```

### diversity metrics

- switch to Shannon exponential 

#### first check coverage 
- wide range in sample sizes (95 - 11000) so check coverage 

- our samples are mostly at 100% coverage - one at 92%, few 95-100%. 
- if we decided to apply coverage dataset, the default paramters would put it at 95%, with most samples rarefied. 
- it is only reccomended to extrapolate up to double the sample size, so would only get to about 200 in size and lose a TON of data
- given the high coverage, I think analyzing raw richness is fine 


```{r}
library(iNEXT)

inverts_all_round <- round(all_inverts)

inverts_all_matrix <- as.matrix(inverts_all_round)

##Convert each row into an abundance vector for iNEXT 
comm_list <- apply(inverts_all_round, 1, function(x) x[x > 0])

#hill_all <- iNEXT::estimateD(
 # comm_list,
#  q = c(0, 1, 2),
#  datatype = "abundance",
#  base = "coverage")

coverage_results <- DataInfo(comm_list, datatype = "abundance")
coverage_results
...
diversity_2021 <- data.frame(sites, Shannon = exp(diversity(a2021, index="shannon")),
                             InverseSimpson = diversity(a2021, index="invsimpson"),
                             richness = specnumber(a2021),
                             Shannon_classic = diversity(a2021, index="shannon")) %>%
        mutate(Evenness = Shannon_classic/log(richness)) %>% 
  select(-Shannon_classic) %>%
  pivot_longer(cols=c("Shannon", "InverseSimpson", "richness", "Evenness"), names_to = "diversity_metric", values_to = "diversity") %>% mutate(year = "2021")

diversity_2022 <- data.frame(sites, Shannon = exp(diversity(a2022, index="shannon")),
                             InverseSimpson = diversity(a2022, index="invsimpson"),
                             richness = specnumber(a2022), 
                             Shannon_classic = diversity(a2022, index="shannon")) %>%
        mutate(Evenness = Shannon_classic/log(richness)) %>% 
  select(-Shannon_classic) %>%
pivot_longer(cols=c("Shannon", "InverseSimpson", "richness", "Evenness"), names_to = "diversity_metric", values_to = "diversity") %>% mutate(year = "2022")

diversity_2023 <- data.frame(sites, Shannon = exp(diversity(a2023, index="shannon")),
                             InverseSimpson = diversity(a2023, index="invsimpson"),
                             richness = specnumber(a2023), 
                             Shannon_classic = diversity(a2023, index="shannon")) %>%
        mutate(Evenness = Shannon_classic/log(richness)) %>% 
  select(-Shannon_classic) %>%
  pivot_longer(cols=c("Shannon", "InverseSimpson", "richness", "Evenness"), names_to = "diversity_metric", values_to = "diversity")%>% mutate(year = "2023")
...
filter_div_sig <- mort_diversity %>%
  filter(year == "2022") %>%
  filter(diversity_metric %in% c("Evenness", "Shannon"))
...
#+ geom_point(data = filter_div, color = "black", fill = "black", size = 4)  
#+ geom_smooth(data = filter_div_sig, method = "lm", color = "#d95f02", fill = "#d95f02") + 
#  geom_point(data = filter_div_sig, color = "#d95f02", fill = "#d95f02", size = 2.1)

plot_div_defo

#ggplot(data = filter_div, aes(x = cdefo5y, y = diversity)) + geom_point(aes(color = mort_cat), fill = "black", size = 4) + scale_color_manual(color = "black")

### change this to 3 separate plots and then after we add the ref range info later on we will combine with ggarrange
## just to be consistent in paper with A B C labels rather than facets 

plot_richness_defo <- mort_diversity %>%
  filter(diversity_metric == "richness") %>%
        ggscatter(x = "cdefo5y", y = "diversity", color = "year", size = 4, shape = "mort_cat",
                  #add = "reg.line", conf.int = TRUE, , cor.coef.coord = c(3, 100), cor.coef.size = 5
                  cor.coef = FALSE, cor.method = "pearson") + 
  stat_cor(cor.coef.name = "r",label.sep = ", ", r.accuracy = 0.01, p.accuracy = 0.01) +
      facet_wrap(~year) + scale_color_brewer(palette = "Dark2") + labs(x = "Cumulative Defoliation", y = "Richness") + theme_bw(base_size = 20) + theme(panel.border = element_blank(), panel.grid.major = element_blank(),
panel.grid.minor = element_blank(), axis.line = element_line(colour = "black"), legend.position = "bottom") + scale_shape_manual(values = c("NoMort" = 16, "Mort" = 17, "LowMort" = 15),  labels = c("NoMort" = "< 10% Mortality", "LowMort" = "10 - 29.9% Mortality", "Mort" = "> 30% Mortality")) + 
 guides(color = guide_legend(title = "Year", nrow = 3, byrow = TRUE),
         shape = guide_legend(title = "Tree Mortality", nrow = 3, byrow = TRUE)) + theme(legend.position = "bottom")
...
plot_evenness_defo <- mort_diversity %>%
  filter(diversity_metric == "Evenness") %>%
        ggscatter(x = "cdefo5y", y = "diversity", color = "year", size = 4, shape = "mort_cat",
                  #add = "reg.line", conf.int = TRUE, , cor.coef.coord = c(3, 100)
                  cor.coef = FALSE, cor.method = "pearson", cor.coef.size = 5) + 
  stat_cor(cor.coef.name = "r",label.sep = ", ", r.accuracy = 0.01, p.accuracy = 0.01, label.y = 0.55) +
      facet_wrap(~year) + scale_color_brewer(palette = "Dark2") + labs(x = "Cumulative Defoliation", y = "Evenness") + theme_bw(base_size = 20) + theme(panel.border = element_blank(), panel.grid.major = element_blank(),
panel.grid.minor = element_blank(), axis.line = element_line(colour = "black"), legend.position = "bottom") + scale_shape_manual(values = c("NoMort" = 16, "Mort" = 17, "LowMort" = 15),  labels = c("NoMort" = "< 10% Mortality", "LowMort" = "10 - 29.9% Mortality", "Mort" = "> 30% Mortality")) + 
  guides(color = guide_legend(title = "Year", nrow = 3, byrow = TRUE),
         shape = guide_legend(title = "Tree Mortality", nrow = 3, byrow = TRUE)) + theme(legend.position = "bottom")
...
plot_shannon_defo <- mort_diversity %>%
  filter(diversity_metric == "Shannon") %>%
        ggscatter(x = "cdefo5y", y = "diversity", color = "year", size = 4, shape = "mort_cat",
                  #add = "reg.line", conf.int = TRUE, , cor.coef.coord = c(3, 100)
                  cor.coef = FALSE, cor.method = "pearson", cor.coef.size = 5) + 
  stat_cor(cor.coef.name = "r",label.sep = ", ", r.accuracy = 0.01, p.accuracy = 0.01) +
  facet_wrap(~year) + scale_color_brewer(palette = "Dark2") + labs(x = "Cumulative Defoliation", y = "Shannon Diversity") + theme_bw(base_size = 20) + theme(panel.border = element_blank(), panel.grid.major = element_blank(),
panel.grid.minor = element_blank(), axis.line = element_line(colour = "black"), legend.position = "bottom") + scale_shape_manual(values = c("NoMort" = 16, "Mort" = 17, "LowMort" = 15),  labels = c("NoMort" = "< 10% Mortality", "LowMort" = "10 - 29.9% Mortality", "Mort" = "> 30% Mortality")) + 
  guides(color = guide_legend(title = "Year", nrow = 3, byrow = TRUE),
         shape = guide_legend(title = "Tree Mortality", nrow = 3, byrow = TRUE)) + theme(legend.position = "bottom")
...
# ok now its ready to actual run the metrics with vegan
#Pielou’s evenness is Shannon diversity (NOT exponential) divided by richness
diversity_refs <- data.frame(sampleid, Shannon = exp(diversity(ainverts1_refs, index="shannon")),
                             InverseSimpson = diversity(ainverts1_refs, index="invsimpson"),
                             richness = specnumber(ainverts1_refs),
                             Shannon_classic = diversity(ainverts1_refs, index="shannon")) %>%
        mutate(Evenness = Shannon_classic/log(richness)) %>% 
  select(-Shannon_classic) %>%
  pivot_longer(cols=c("Shannon", "InverseSimpson", "richness", "Evenness"), names_to = "diversity_metric", values_to = "diversity") %>%
  filter(site != "0")
...
diversity_refs_evenness <- diversity_refs_summary %>%
  filter(diversity_metric == "Evenness")

diversity_refs_richness <- diversity_refs_summary %>%
  filter(diversity_metric == "richness")

diversity_refs_shannon <- diversity_refs_summary %>%
  filter(diversity_metric == "Shannon")

## Try this with the individual plots
#diversity and defo 
plot_even_ref <- plot_evenness_defo + geom_hline(data = diversity_refs_evenness, aes(yintercept=mean), linetype="dashed", color = "red", size=1) + geom_rect(data = diversity_refs_evenness, mapping = aes(xmin=-Inf, xmax = Inf, ymin = low.limit, ymax = up.limit, x= NULL, y = NULL), alpha=.4, fill='grey') 

plot_rich_ref <- plot_richness_defo + geom_hline(data = diversity_refs_richness, aes(yintercept=mean), linetype="dashed", color = "red", size=1) + geom_rect(data = diversity_refs_richness, mapping = aes(xmin=-Inf, xmax = Inf, ymin = low.limit, ymax = up.limit, x= NULL, y = NULL), alpha=.4, fill='grey') 

plot_shan_ref <- plot_shannon_defo + geom_hline(data = diversity_refs_shannon, aes(yintercept=mean), linetype="dashed", color = "red", size=1) + geom_rect(data = diversity_refs_shannon, mapping = aes(xmin=-Inf, xmax = Inf, ymin = low.limit, ymax = up.limit, x= NULL, y = NULL), alpha=.4, fill='grey') 

## gg arrange for ABC labels
final_diversity_plot <- ggarrange(plot_even_ref, plot_rich_ref, plot_shan_ref,
        labels = c("A", "B", "C"),
         font.label=list(size = 16, color = "black", face = "bold"),
     common.legend = TRUE,
         legend = "bottom",
...
final_diversity_plot

```

### Export High Quality Figs

```{r}
setwd("C:/Users/mmccaig/OneDrive - NRCan RNCan/Documents/GitHub/SpruceBudworm_Stream-Bioindicator-Assessment/FinalFigs/")

ggsave("Fig1_BioindicatorPaper.jpg", plot = plot_pca_all1, width = 18.2, height = 20, dpi = 300, units = "cm", scale = .9, bg = "white")

ggsave("Fig2_BioindicatorPaper.jpg", plot = final_plot, width = 18.2, height = 15, dpi = 300, units = "cm", scale = 1.3, bg = "white")

ggsave("Fig3_BioindicatorPaper.jpg", plot = final_pca_all_plot, width = 18.2, height = 20, dpi = 300, units = "cm", scale = 1, bg = "white")

ggsave("Fig4_BioindicatorPaper.jpg", plot = final_diversity_plot, width = 18.2, height = 20, dpi = 300, units = "cm", scale = 1.4, bg = "white")

ggsave("Fig5_BioindicatorPaper.jpg", plot = EPT_defo_final, width = 18.2, height = 20, dpi = 300, units = "cm", scale = 1.4, bg = "white")

ggsave("Fig6_BioindicatorPaper.jpg", plot = EPT_mort_final, width = 8.84, height = 10, dpi = 300, units = "cm", scale = 2)
