...
- switch to Hill based metrics with iNEXT to account for coverage 

```{r}
library(iNEXT)

inverts_round <- inverts %>%
  mutate(count = round(count))

inv_mat <- inverts_round %>%
  filter(year != "2019") %>%
  select(Family, site, year, count) %>%
  group_by(Family, site, year) %>%
  summarize(count = sum(count), .groups = "drop") %>%
  unite(site_year, site, year, sep = "_") %>%   # keep site-year unique
  pivot_wider(names_from = Family, values_from = count, values_fill = 0)

inv_mat_df <- as.data.frame(inv_mat)
rownames(inv_mat_df) <- inv_mat_df$site_year
inv_mat_df <- inv_mat_df %>% select(-site_year)

##Convert each row into an abundance vector for iNEXT 
comm_list <- apply(inv_mat_df, 1, function(x) x[x > 0])
...
hill_all <- hill_all %>%
  separate(Assemblage, into = c("site", "year"), sep = "_", remove = FALSE) %>%
  rename(diversity_metric = Order.q, diversity = qD)

hill_all <- hill_all %>%
  mutate(diversity_metric = ifelse(diversity_metric == 0, "richness", 
                                  ifelse(diversity_metric == 1, "Shannon", "InverseSimpson")))

#calculate Hill Shannon Evenness and Simpsons Evenness
#shannon sensitive to common species # simpson strongly weights dominant species 


hill_wide <- hill_all %>%
  select(Assemblage, diversity_metric, diversity) %>%
  pivot_wider(names_from = diversity_metric, values_from = diversity)

hill_evenness <- hill_wide %>%
  mutate(
    Shannon_Evenness = Shannon / richness,   # Shannon evenness (J')
    InvSimp_Evenness = InverseSimpson / richness    # Simpson-based evenness
  )

hill_long <- hill_evenness %>%
  pivot_longer(cols = c(richness, Shannon, InverseSimpson , Shannon_Evenness, InvSimp_Evenness),
...
hill_long$site <- hill_long$site %>%    
        factor(levels = c("U01", "U02", "U03", "C04", "C05", "C06", "C07", "L08", "L09", "L10", "L11", "L12"))

mort_hilldiversity <- mortality %>%
 full_join(hill_long, by = c("site", "year")) %>%
...
diversity_labs <- c(`InverseSimpson` = "Inverse Simpson's",
                    ...
                    `Shannon_Evenness` = "Evenness (Shannon-based)",
                    `InvSimp_Evenness` = "Evenness (Simpson-based)")

mort_hilldiversity$diversity_metric <- mort_hilldiversity$diversity_metric %>%
                  factor(levels = c("richness", "Shannon", "InverseSimpson", "Shannon_Evenness", "InvSimp_Evenness"))

filter_div <- mort_hilldiversity %>%
 # filter(diversity_metric %in% c("Shannon", "Evenness", "richness")) %>%
  filter(site %in% c("U01", "U02")) %>%
  filter(year == "2023")

filter_div_sig <- mort_hilldiversity %>%
  filter(year == "2022") %>%
...
mort_hilldiversity <- mort_hilldiversity %>%
mutate(mort_cat = ifelse(site == "U01" & year == "2022", "LowMort",       
                    ifelse( year == "2023" & site %in% c("U01", "U02"), "Mort", "NoMort")))

mort_hilldiversity$mort_cat <- factor(mort_hilldiversity$mort_cat, levels = c("NoMort", "LowMort", "Mort"))

mort_hilldiversity <- mort_hilldiversity %>%
  left_join(defo_final, by = c("site", "year"))

# defo diversity
plot_div_defo <- mort_hilldiversity %>%
  filter(diversity_metric %in% c("Shannon", "Shannon_Evenness", "richness")) %>%
      ...
      facet_grid(diversity_metric~year, scales = "free_y", labeller = labeller(diversity_metric = diversity_labs)) + scale_color_brewer(palette = "Dark2") + labs(x = "Cumulative Defoliation", y = "Diversity") + theme_bw(base_size = 20) + theme(panel.border = element_blank(), panel.grid.major = element_blank(),
...
plot_richness_defo <- mort_hilldiversity %>%
...
plot_evenness_defo <- mort_hilldiversity %>%
  filter(diversity_metric == "Shannon_Evenness") %>%
        ggscatter(x = "cdefo5y", y = "diversity", color = "year", size = 4, shape = "mort_cat",
                  #add = "reg.line", conf.int = TRUE, , cor.coef.coord = c(3, 100)
                  cor.coef = TRUE, cor.method = "pearson", cor.coef.size = 5) + 
...
plot_shannon_defo <- mort_hilldiversity %>%
...
- updated to iNEXT coverage based

```{r}
invertrefs_round <- invert_refs %>%
  mutate(count = round(count))

invref_mat <- invertrefs_round %>%
  select(Family, site, year, count, SampleId) %>%
  group_by(Family, SampleId) %>%
  summarize(count = sum(count), .groups = "drop") %>%
  #unite(site_year, site, year, sep = "_") %>%   # keep site-year unique
  pivot_wider(names_from = Family, values_from = count, values_fill = 0)

invref_mat_df <- as.data.frame(invref_mat)
rownames(invref_mat_df) <- invref_mat_df$SampleId
invref_mat_df <- invref_mat_df %>% select(-SampleId)

##Convert each row into an abundance vector for iNEXT 
comm_list <- apply(invref_mat_df, 1, function(x) x[x > 0])

hill_all_refs <- iNEXT::estimateD(
  comm_list,
  q = c(0, 1, 2),
  datatype = "abundance",
  base = "coverage"
...
hill_wide_refs <- hill_all_refs %>%
  select(Assemblage, diversity_metric, diversity) %>%
  pivot_wider(names_from = diversity_metric, values_from = diversity)

hill_evenness_refs <- hill_wide_refs %>%
  mutate(
    Shannon_Evenness = Shannon / richness,   # Shannon evenness (J')
    InvSimp_Evenness = InverseSimpson / richness    # Simpson-based evenness
  )

hill_long_refs <- hill_evenness_refs %>%
  pivot_longer(cols = c(richness, Shannon, InverseSimpson , Shannon_Evenness, InvSimp_Evenness),
               names_to = "diversity_metric",
               values_to = "diversity")

diversity_refs_summary <- hill_long_refs %>%
...
group <- factor(c(rep("Defo", 36), rep("Non", 49)))
