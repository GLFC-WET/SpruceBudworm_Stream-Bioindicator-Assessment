...
# try mvabund 
# this is not affected by dispersion, also gives us some individual taxa responses
#couldnt use manyGLM because it only takes raw counts - need manyLM that allows for the transformed data 

library(mvabund)

# samples in rows, species in cols 

comm_mv <- mvabund(all_inverts_hel)
...
### now there are several suspect entries in this dataset
### run any analysis with and without them 
### there are several depth values (mean and max) which are below 1.
# confirmed with project lead that these were indeed reported as metres, but also rounded. they are going to provide us with updated data soon
### also several bankful and wetted width values that are questionable (e.g. 25m bankful with wetted of 5 in one year and 13m bankful with 5 in next)

### run analysis with and without suspect values 
RefHabitat_confident <- RefHabitat %>%
  filter(suspect == "no")

```

```{r}
# confident reference habitat data we have 36 samples. full dataset is 51
...
```{r}
RefHabitat_confident <- RefHabitat_confident %>%
...
loadings1 <- loadings %>%
  filter(env.variables %in% c("wetted_width.m", "depth_mean.m", "Elevation_Mean_metres"))
...
ggplot() +
  ...
  stat_ellipse(data = scores, aes(x = PC1, y = PC2, linetype = Site_Type, color = Site_Type), type = "norm", level = 0.95) + geom_text_repel(data = loadings1, aes(PC1*2, PC2*2, colour = "black", fontface = "bold", label = pretty_label), size =4) + geom_text(data = loading_temp, aes(PC1*2, PC2*2, colour = "black", fontface = "bold", label = pretty_label, nudge_y = 0.1), size =4) + geom_text(data = loading_catch, aes(PC1*2, PC2*2, colour = "black", fontface = "bold", label = pretty_label, nudge_x = -0.7), size =4) + geom_text(data = loading_pH, aes(PC1*2, PC2*2, colour = "black", fontface = "bold", label = pretty_label, nudge_y = -0.1), size =4) + geom_text(data = loading_spfir, aes(PC1*2, PC2*2, colour = "black", fontface = "bold", label = pretty_label, nudge_x = 0.6), size =4) + geom_text(data = loading_flow, aes(PC1*2, PC2*2, colour = "black", fontface = "bold", label = pretty_label, nudge_x = -0.6), size =4)
